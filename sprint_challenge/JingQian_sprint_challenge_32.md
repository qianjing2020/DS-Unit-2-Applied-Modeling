...
answer: Table Employee is not directly related to Table Territory by through another table which is Table EmployeeTerritories. Relationship: Table Employee to Table EmployeeTerritories is one-to-many; and Table EmployeeTerritory to Table Territory is one-to-one.  
        
- What is a situation where a document store (like MongoDB) is appropriate, and  what is a situation where it is not appropriate?

answer: Document stores such as MongoDB are NoSQL system which can stores documents (the smallest unit that make up a collection in their database). 
It is appropriate to use document store when the data have no fixed structure, data are nested or embedded. In another words, the documents can have different fields, and/or fields contain arrays and/or sub-documents. 
Document stores won't be appropriate when the data has a fixed structure, in which case the traditional SQL can store them more efficietly.

- What is "NewSQL", and what is it trying to achieve?

answer: NewSQL are relational database systems that is compliant with the ACID code of traditional SQL DBMS and has the same scalability of NoSQL. It trys to achieve improved performance, avalability, scalability, and stability with huge amount of complex data through new features such as partitioning/sharding, replication, secondary indexes, concurrency control, and crash recovery. 
